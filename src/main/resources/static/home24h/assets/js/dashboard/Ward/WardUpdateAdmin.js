         ...
         $(document).ready(function () {
            ...
            $("#btn-update").on("click", function () {
               ...
               if (vValidate) {
                  ...
                  let vWardIdUrl = vUrlStr.searchParams.get("wardId");

                  callApiUpdateDistrict(vData, vWardIdUrl)
                     .then(function (res) {
                        handleSuccess();
                     })
                     .catch(function (xhr) {
            ...
            // ------------------------------ CALL API ----------------------------
            function callApiUpdateDistrict(paramData, paramWardId) {
               return new Promise(function (resolve, reject) {
                  $.ajax({
                     type: "PUT",
                     url: `/district/put/${paramWardId}/${paramData.districtId}/${paramData.provinceId}`,
                     headers: gHeader,
            ...
            //Gọi tất cả All Province
            function callApiWardById(paramWardId) {
               return new Promise(function (resolve, reject) {
                  $.ajax({
                     type: "GET",
                     headers: gHeader,
                     url: `/ward/get/wardById/${paramWardId}`,
                     success: function (res) {
                        resolve(res);
                     },
                     error: function (xhr) {
            ...
            async function handleLoading() {
               let vUrlStr = new URL(window.location.href);
               let vWardIdUrl = vUrlStr.searchParams.get("wardId");
               let [vProvince, vWard] = await Promise.all([callApiAllProvince(), callApiWardById(vWardIdUrl)]);
               loadSelectProvince(vProvince);
               $("#inp-name").val(vWard.name);
               // $("#select-province").val(vDistrict.idProvince);
               $("#select-prefix option")
                  .filter(function () {
                     return $(this).text() === vWard.prefix;
                  })
                  .prop("selected", true);

               $("#select-province").val(vWard.provinceById);

               let vDistrict = await callApiDistrictByProvinceId(vWard.provinceById);
               loadSelectDistrict(vDistrict);
               $("#select-district").val(vWard.districtById);
            }
