   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            var vTable = $("#utilities-table").DataTable({});
            ...
            // --------------------- UPDATE --------------------------------
            $("#utilities-table").on("click", ".btn-update", function (event) {
               event.stopPropagation();
               getDataRow(this);
               window.location.href = `../UtilitiesAdmin/UtilitiesUpdateAdmin.html?id=${gUtilitiesIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#utilities-table").on("click", ".btn-delete", function (event) {
               event.stopPropagation();
               getDataRow(this);
               $("#deleteUtilities-modal").modal("show");
            });

            // --------------------- DETAIL --------------------------------
            //Khi click vào tr trên table
            $("#utilities-table").on("click", "tr", function () {
            ...
            function callApiDeleteUtilities() {
               $.ajax({
                  url: `/utilities/delete/${gUtilitiesIdClick}`,
                  headers: gHeader,
                  type: "DELETE",
                  success: function (res) {
                     $("#deleteUtilities-modal").modal("hide");
            ...
            function loadTable(url) {
               ...
               vTable.destroy();
               return $("#utilities-table").DataTable({
                  processing: false,
                  serverSide: true,
                  autoWidth: true,
                  searching: false,
                  ...
                  columns: [{ data: "id" }, { data: "name" }, { data: "Cập Nhật" }],
                  columnDefs: [
                     { targets: [0, 1, 2], class: "text-center" },
                     {
                        targets: 0,
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/utilities/get/allUtilities?name=${vName}`;
               return vUrl;
            }
         });
      } catch (error) {
