   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            var vTable = $("#street-table").DataTable({});
            ...
            // --------------------- UPDATE --------------------------------
            $("#street-table").on("click", ".btn-update", function (event) {
               getDataRow(this);
               window.location.href = `../StreetAdmin/StreetUpdateAdmin.html?streetId=${gStreetIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#street-table").on("click", ".btn-delete", function (event) {
               getDataRow(this);
               $("#deleteStreet-modal").modal("show");
            });

            //Khi ấn nút xóa trên modal
            $("#btn-delete-modal").on("click", function () {
               callApiDeleteStreet(gStreetIdClick).then(function (res) {
                  toastr.success("Xóa Thành Công");
                  $("#deleteStreet-modal").modal("hide");
            ...
            function callApiDeleteStreet(paramId) {
               return new Promise(function (resolve, reject) {
                  $.ajax({
                     type: "DELETE",
                     url: `/street/delete/${paramId}`,
                     headers: gHeader,
                     success: function (res) {
                        resolve(res);
                     },
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/street/get/allDetailStreet?provinceId=${vProvinceId}&districtId=${vDistrictId}`;
               return vUrl;
            }
