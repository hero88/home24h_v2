   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            var vTable = $("#contractor-table").DataTable({});

            /*** REGION 1 - Global variables - Vùng khai báo biến, hằng số, tham số TOÀN CỤC */
            var gContractorIdClick;
            ...
            // --------------------- UPDATE --------------------------------
            $("#contractor-table").on("click", ".btn-update", function (event) {
               event.stopPropagation();

               getDataRow(this);
               window.location.href = `../ContractorAdmin/ContractorUpdateAdmin.html?id=${gContractorIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#contractor-table").on("click", ".btn-delete", function (event) {
               event.stopPropagation();
               getDataRow(this);
               $("#deleteContractor-modal").modal("show");
            });

            //Khi ấn nút xóa trên modal
            $("#btn-delete-modal").on("click", function () {
               callApiDeleteContractor();
            });

            // --------------------- DETAIL --------------------------------
            //Khi clickl vào dòng trên table
            $("#contractor-table").on("click", "tr", function (event) {
            ...
            function callApiDeleteContractor() {
               $.ajax({
                  url: `/contractor/delete/${gContractorIdClick}`,
                  headers: gHeader,
                  type: "DELETE",
                  success: function (res) {
                     $("#deleteContractor-modal").modal("hide");
            ...
            function loadTable(url) {
               ...
               vTable.destroy();
               return $("#contractor-table").DataTable({
                  processing: false,
                  serverSide: true,
                  autoWidth: false,
                  searching: false,
            ...
            function getDataRow(paramBtn) {
               ...
               gContractorIdClick = vDataRow.id;
               return vDataRow;
            }
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/contractor/get/allContractor?name=${vName}`;
               return vUrl;
            }
         });
      } catch (error) {
