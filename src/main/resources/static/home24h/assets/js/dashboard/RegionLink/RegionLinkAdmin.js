   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            var vTable = $("#regionLink-table").DataTable({});
            ...
            // --------------------- UPDATE --------------------------------
            $("#regionLink-table").on("click", ".btn-update", function (event) {
               event.stopPropagation();
               getDataRow(this);
               window.location.href = `../RegionLinkAdmin/RegionLinkUpdateAdmin.html?id=${gRegionLinkIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#regionLink-table").on("click", ".btn-delete", function (event) {
               event.stopPropagation();
               getDataRow(this);
               $("#deleteRegionLink-modal").modal("show");
            });

            // --------------------- DETAIL --------------------------------
            //Khi click vào tr trên table
            $("#regionLink-table").on("click", "tr", function () {
            ...
            function callApiDeleteRegionLink() {
               $.ajax({
                  url: `/regionLink/delete/${gRegionLinkIdClick}`,
                  headers: gHeader,
                  type: "DELETE",
                  success: function (res) {
                     $("#deleteRegionLink-modal").modal("hide");
            ...
            function loadTable(url) {
               ...
               vTable.destroy();
               return $("#regionLink-table").DataTable({
                  processing: false,
                  serverSide: true,
                  autoWidth: true,
                  searching: false,
                  ...
                  columnDefs: [
                     ...
                     },
                     {
                        ...
                        render: function (data) {
                           return `<img src="/images/${data}" style="width:150px">`;
                        },
                     },
                     {
                        targets: 4,
            ...
            function getDataRow(paramBtn) {
               ...
               gRegionLinkIdClick = vDataRow.id;
               return vDataRow;
            }
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/regionLink/get/allRegionLink?name=${vName}`;
               return vUrl;
            }
         });
      } catch (error) {
