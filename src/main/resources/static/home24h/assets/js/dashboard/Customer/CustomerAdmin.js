   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            ...
            // --------------------- SEARCH --------------------------------
            $("#inp-searchPhone").on("input", function () {
               vTable = loadTable(getDataFindToUrl());
            });

            // --------------------- UPDATE --------------------------------
            $("#customer-table").on("click", ".btn-update", function (event) {
               event.stopPropagation();
               getDataRow(this);
               window.location.href = `../CustomerAdmin/CustomerUpdateAdmin.html?id=${gCustomerIdClick}`;
            });
            ...
            function callApiDeleteCustomer() {
               $.ajax({
                  url: `/customer/delete/${gCustomerIdClick}`,
                  headers: gHeader,
                  type: "DELETE",
                  success: function (res) {
            ...
            function getDataFindToUrl() {
               let vPhone = $("#inp-searchPhone").val();
               let vUrl = `/customer/get/allCustomer?phone=${vPhone}`;
               return vUrl;
            }
         });
      } catch (error) {
