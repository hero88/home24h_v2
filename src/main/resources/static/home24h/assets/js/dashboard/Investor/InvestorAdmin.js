   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            ...
            // --------------------- UPDATE --------------------------------
            $("#investor-table").on("click", ".btn-update", function (event) {
               event.stopPropagation();
               getDataRow(this);
               window.location.href = `../InvestorAdmin/InvestorUpdateAdmin.html?id=${gInvestorIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#investor-table").on("click", ".btn-delete", function (event) {
               event.stopPropagation();
               getDataRow(this);
               $("#deleteInvestor-modal").modal("show");
            });

            //Khi ấn nút xóa trên modal
            $("#btn-delete-modal").on("click", function () {
               callApiDeleteInvestor();
            });

            // --------------------- DETAIL --------------------------------
            //Khi ấn nút xóa trên table
            $("#investor-table").on("click", "tr", function () {
            ...
            function callApiDeleteInvestor() {
               $.ajax({
                  url: `/investor/delete/${gInvestorIdClick}`,
                  headers: gHeader,
                  type: "DELETE",
                  success: function (res) {
                     $("#deleteInvestor-modal").modal("hide");
            ...
            function loadTable(url) {
               ...
               vTable.destroy();
               return $("#investor-table").DataTable({
                  processing: false,
                  serverSide: true,
                  autoWidth: false,
                  searching: false,
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/investor/get/allInvestor?name=${vName}`;
               return vUrl;
            }
         });
      } catch (error) {
