      ...
      try {
         ...
         $(document).ready(function () {
            ...
            function onPageLoading() {
               ...
               //Lấy id DesignUnit trên Url
               let vUrl = window.location.href;
               let vUrlString = new URL(vUrl);
               gDesignUnitIdClick = vUrlString.searchParams.get("id");
               //gọi Api DesignUnit by Id
               callApiDesignUnitById(gDesignUnitIdClick);
            }

            /*** REGION 4 - Common funtions - Vùng khai báo hàm dùng chung trong toàn bộ chương trình*/
            // ------------------------------ CALL API ----------------------------------------------
            //Gọi Api tạo DesignUnit
            function callApiUpdateContactor(paramData) {
               $.ajax({
                  type: "PUT",
                  headers: gHeader,
                  url: `/designUnit/put/${gDesignUnitIdClick}`,
            ...
            //Gọi API DesignUnit bới Id
            function callApiDesignUnitById(paramId) {
               $.ajax({
                  type: "GET",
                  headers: gHeader,
                  url: `/designUnit/get/${paramId}`,
                  success: function (res) {
                     loadDataToForm(res);
                  },
               });
