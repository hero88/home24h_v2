   ...
   if (gInfoUser == null) {
      ...
      try {
         ...
         $(document).ready(function () {
            var vTable = $("#district-table").DataTable({});

            /*** REGION 1 - Global variables - Vùng khai báo biến, hằng số, tham số TOÀN CỤC */
            let gDistrictIdClick;
            ...
            // --------------------- UPDATE --------------------------------
            $("#district-table").on("click", ".btn-update", function (event) {
               getDataRow(this);
               window.location.href = `../DistrictAdmin/DistrictUpdateAdmin.html?districtId=${gDistrictIdClick}`;
            });

            // --------------------- DELETE --------------------------------
            //Khi ấn nút xóa trên table
            $("#district-table").on("click", ".btn-delete", function (event) {
               getDataRow(this);
               $("#deleteDistrict-modal").modal("show");
            });

            //Khi ấn nút xóa trên modal
            $("#btn-delete-modal").on("click", function () {
               callApiDeleteDistrict(gDistrictIdClick).then(function (res) {
                  toastr.success("Xóa Thành Công");
                  $("#deleteDistrict-modal").modal("hide");
            ...
            function loadTable(url) {
               vTable.destroy();
               return $("#district-table").DataTable({
                  processing: false,
                  serverSide: true,
                  autoWidth: false,
                  searching: false,
                  ...
                  columnDefs: [
                     { targets: [0, 1, 2, 3, 4, 5, 6], class: "text-center" },
                     {
                        ...
                        render: function (data, type, row, meta) {
                           return `<a   href="../ProjectAdmin/ProjectAdmin.html?provinceId=${row.idProvince}&districtId=${row.id}" class="btn btn-info">${data}</a>`;
                        },
                     },

                     {
                        targets: 2,
                        render: function (data, type, row, meta) {
                           return `<a  href="../RealestateAdmin/realestateAdmin.html?provinceId=${row.idProvince}&districtId=${row.id}" class="btn btn-info">${data}</a>`;
                        },
                     },
            ...
            function getDataFindToUrl() {
               ...
               let vUrl = `/district/get/allDetailDistrict?name=${vName}&sort=${vSort}`;
               return vUrl;
            }
