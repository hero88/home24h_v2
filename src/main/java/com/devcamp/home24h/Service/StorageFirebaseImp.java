...
public class StorageFirebaseImp implements StorageFirebaseService {
    ...
    @Override
    public byte[] downloadImage(String floder, String imageName) {
        try {
            String pathFull = floder + "/" + imageName;
            return StorageClient.getInstance().bucket().get(pathFull).getContent();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
